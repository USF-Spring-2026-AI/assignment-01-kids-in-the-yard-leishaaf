...
class PersonFactory:
    ...
    def set_is_descendant(self, is_descendant):
        self.is_descendant = is_descendant 

    def set_person_one_lastname(self, person_one_lastname):
        self.person_one_lastname = person_one_lastname

    def set_person_two_lastname(self, person_two_lastname):
        self.person_two_lastname = person_two_lastname
    
    def get_is_descendant(self):
        return self.is_descendant

    def read_files(self):
        """ Reads all the csv files and stores the data frame for each """
        print("Reading files...")
    ...
    def calc_life_expectancy(self, year_born):
        """ Calculates the life expectancy following the rules of the spec """
    ...
    def generate_first_name(self, year_born):
        """ Generated the first name of person using random choices and weight given in csv """
    ...
    def generate_last_name(self, year_born):
        """ If not a descendant, generates the last name of person using random choices and weight given in csv. Otherwise takes the lastname of first two people"""
        last_name = ""
        if self.is_descendant and self.person_one_lastname is not None:
    ...
    def calc_marriage(self, year_born,):
        """ Calculates if person will have a partner or not given marriage weight """
        str_year_born = str(year_born)
    ...
    def generate_partner(self, person, year_born):
            """ Generates a partner given the curr persons year of birth """
            ...
            # set all the partners attributes
    ...
    def calc_children(self, person, year_born):
        """ Returns the amount of children after using a weighted probability span given csv """
        str_year_born = str(year_born)
        str_year_born = str_year_born[0:3]
        decade_mask = self.birth_and_marriage_df['decade'].str.contains(str_year_born)
        birth_rate = self.birth_and_marriage_df.loc[decade_mask, 'birth_rate'].iloc[0] # .iloc[0] from ChatGpt
        min_children = max(1, math.floor(birth_rate - 1.5)) # ChatGpt because I was somehow still getting 0 amount of children with my implementation
        max_children = max(1, math.ceil(birth_rate + 1.5)) # ^^
        amount_children = random.randint(min_children, max_children)
    ...
    def generate_children(self, person, year_born, children_amount):
        """ Generates the children in an evenly distributed way given the calculated children amount and elder parents year of birth """
